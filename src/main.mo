...
shared(_init_msg) actor class Example(_args : {
  ...
  stable var signer: Principal = Principal.fromText("aaaaa-aa");
  stable var id_counter: Nat = 10999999;
  stable var base_uri: Text = "https://townstory.io/nft/ticket/";
  ...
  public shared(msg) func mint_pass(to: ICRC7.Account, md: Text, tempzone: Text) : async ([ICRC7.SetNFTResult], Nat) {
    if(msg.caller != icrc7().get_state().owner) D.trap("Unauthorized");

    id_counter += 1;
    var baseNFT: CandyTypesLib.CandyShared = #Class([
      {immutable=false; name="url"; value = #Text("");}
    ]);

    if (Text.size(md) > 0) {
      let unstable: CandyTypesLib.Candy = #Text(md);
      baseNFT := CandyTypesLib.shareCandy(unstable);
    } else {
      baseNFT := #Class([
      { 
          immutable=true;
          name="icrc7:metadata:uri:image";
          value = #Text(base_uri # tempzone # ".png");
      },
      { 
          immutable=true;
          name="icrc7:metadata:TempZone";
          value = #Text(tempzone);
      }]);
    };
    
    let tokens: ICRC7.SetNFTRequest = [{
    ...
    switch(icrc7().set_nfts<system>(msg.caller, tokens, true)){
      case(#ok(val)) (val, id_counter);
      case(#err(err)) D.trap(err);
    };
  };

  public shared query func get_collection_owner() : async Principal {
    icrc7().get_collection_owner();
  };

  public shared query func get_base_uri() : async Text {
    base_uri;
  };

  public shared(msg) func set_base_uri(uri: Text) : async () {
    if(msg.caller != icrc7().get_state().owner) D.trap("Unauthorized");
    base_uri := uri;
  };

  public shared(msg) func transfer_signer(_signer: Principal) : async () {
    if(msg.caller != icrc7().get_state().owner) D.trap("Unauthorized");
    signer := _signer;
  };

  public query func get_current_token_id() : async Nat {
    return id_counter;
  };
